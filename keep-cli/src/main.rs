...
fn run(out: &Output) -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Migrate { command } => dispatch_migrate(out, &path, command),
    }
}
...
fn dispatch_migrate(
    ...
    match command {
        None => commands::migrate::cmd_migrate(out, path),
        Some(MigrateCommands::Status) => commands::migrate::cmd_migrate_status(out, path),
    }
}
