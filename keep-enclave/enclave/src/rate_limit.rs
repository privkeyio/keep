...
impl RateLimiter {
    ...
    pub fn record(&self, key_id: &str) -> bool {
        let Ok(mut minute) = self.minute_buckets.lock() else {
            return false;
        };
        let Ok(mut hour) = self.hour_buckets.lock() else {
            return false;
        };
        let Ok(mut day) = self.day_buckets.lock() else {
            return false;
        };
        minute.record(key_id, 60);
        hour.record(key_id, 600);
        day.record(key_id, 5000);
        true
    }
