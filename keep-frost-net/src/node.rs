...
impl KfpNode {
    ...
    async fn handle_announce(&self, pubkey: PublicKey, payload: AnnouncePayload) -> Result<()> {
        ...
        if let Some(ref expected) = self.expected_pcrs {
            ...
            debug!(
                share_index = payload.share_index,
                pcr0 = %hex::encode(&expected.pcr0[..8]),
                "Peer attestation verified"
            );
        }
...
mod tests {
    ...
    fn test_expected_pcrs_from_hex() {
        let pcr0 = "0".repeat(96); // 48 bytes = 96 hex chars
    ...
    fn test_expected_pcrs_from_hex_invalid_length() {
        let pcr0 = "00".repeat(47); // Wrong length
        let pcr1 = "1".repeat(96);
        let pcr2 = "2".repeat(96);
